...
for n in {01..04}; do
    for T in {005,010,020,030,040,050,060,070,080}; do
        ansible-playbook -v -f 10 -i hosts latencyvsthroughput.yml -e "backend=gorums throughput=$T output=${n} maxentries=64 clients=200 time=60s order=true";
    done;
done;
...
for n in {01..04}; do
    for T in {005,010,020,030,040,050,060,070,080}; do
        ansible-playbook -v -f 10 -i hosts latencyvsthroughput.yml -e "backend=gorums throughput=$T output=${n} maxentries=64 clients=200 time=60s";
    done;
done;
